  ...
  return (
    ...
    >
      ...
      <p>
        ...
        strictly for comparison, not as a change to the default local pipeline. Two real, live examples on the
        same case (<code>isCurrency</code>, from the <code>validator.js</code> corpus), one per backend:
      </p>
      <ul>
        <li>
          <a href="/deobfuscator/isCurrency-qwen.html" target="_blank" rel="noopener noreferrer">Pipeline visualizer: local qwen2.5-coder:7b &rarr;</a>
        </li>
        <li>
          <a href="/deobfuscator/isCurrency-claude.html" target="_blank" rel="noopener noreferrer">Pipeline visualizer: Claude backend (comparison run) &rarr;</a>
        </li>
      </ul>
      <p>
        The project's own documentation already flags the natural next step: this same call-graph-ranking and
        rename-map approach generalizes past JavaScript deobfuscation to any task where "propose identifiers,
        verify behavior didn't change" is the right shape: that's an open direction, not something built yet.
      </p>
    </PostLayout>
  );
}
